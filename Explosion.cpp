...
void Explosion_Create(int type, t_Vector2f loc, t_Vector2f vel) {
	...
	Sparks_Create(theType->spark_Type, theType->spark_Count, loc, vel,
	              theType->spark_Speed);
	...
	for (i = 0; i < (theType->circle_Count); i++) {
		...
		explosion->loc = loc;
		explosion->loc += t_Vector2f(SignedFloatRand, SignedFloatRand) *
						  theType->circle_Scatter;
		explosion->vel = vel;

		explosion->radius = 0;
		explosion->expansion = theType->circle_Speed +
							   UnsignedFloatRand * theType->circle_SpeedRand;
		...
		explosion->nextThink =
			gCurrentTime + ((Random() & 0x7FFF) % (theType->circle_Delay + 1)) +
			1;
...
void Explosion_Draw(t_Entity *e) {
	...
	c = floor(e->radius * 15.0 /
	          gExplosionTypes[e->explosionType].circle_MaxSize);
