...
UInt8 gPltt_HotSparks[18] =		{  0,100,101,102,103,104,105, 29,209,208, 22, 21, 20, 19, 18, 17, 16,  1};
...
void Update_Sparks (void)
{
	...
	for (i=0;i<kMax_SparkGroups;i++)
	{
		if (curSG->exists)
		{
			if ((gCurrentTime - curSG->startTime) >= gSparkTypes[curSG->sparkType].duration)
			{
...
void Draw_Sparks (void)
{
	...
	for (i=0;i<kMax_SparkGroups;i++)
	{
		if (curSG->exists)
		{
			...
			curSP = curSG->sparks;
			pixelData = curST->colors[((gCurrentTime-curSG->startTime)/(curST->period)) % (curST->numColors)];
			...
			if (curST->size == 1)
			{
				for (j=0;j<curSG->count;j++)
				{
					...
					if ((loc.h > gBufferRect.left) &&
						...
						*(baseAddress + bytesPerRow * loc.v + loc.h) = pixelData;
					curSP++;
				}
			} else {
				for (j=0;j<curSG->count;j++)
