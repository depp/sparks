...
void Create_Off_Screen_Bit_Map(Rect *theRect, GrafPtr *offScreen) {
	...
	OpenPort(theBWPort);                             // Open the por
	theRowBytes = (long)((theRect->right - theRect->left + 15L) / 16L) *
				  2L;                        // Figure out size
	theBitMap.rowBytes = (short)theRowBytes; // and other stuff
	theBitMap.baseAddr =
		NewPtr((long)theBitMap.rowBytes * (theRect->bottom - theRect->top));
...
void CopyToScreen(void) {
	...
	CopyBits((BitMap *)*gPixMap, &(gWindPtr->portBits), &gBufferRect,
	         &gGameRect, srcCopy,
...
void Graphics_LoadColors(void) {
	...
	if (gPalette == NULL) {
		gPalette = NewPalette((**gCTable).ctSize + 1, gCTable,
		                      pmTolerant + pmExplicit, 0x0000);
	} else {
