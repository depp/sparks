		...
		if (pa.v == pb.v) {
			...
			if (pb.v == pc.v) {
				...
				if ((pb.h < pa.h) && (pb.h < pc.h)) {
					...
					if (pb.h < ((pb.v - pa.v)*(pc.h - pa.h))/(pc.v - pa.v)) {
						nl = 3; nr = 2;
						l[1] = pb; r[1] = l[2] = pc;
					} else {
						nr = 3; nl = 2;
		...
		while (VLoc <= l[nl-1].v) {
			if (!(VLoc == l[nl-1].v)) {
				if (VLoc == l[left_CurPoint].v) {
					...
					left_Dir = (left_DeltaH > 0)?1:((left_DeltaH < 0)?-1:0);
				...
				if (VLoc == r[right_CurPoint].v) {
					...
					right_DeltaH = r[right_CurPoint + 1].h - r[right_CurPoint].h;
					right_DeltaV = r[right_CurPoint + 1].v - r[right_CurPoint].v;
					right_Dir = (right_DeltaH > 0)?1:((right_DeltaH < 0)?-1:0);
			...
			if (VLoc > gBufferRect.bottom) return;
		...
		while (VLoc <= l[nl-1].v) {
			if (!(VLoc == l[nl-1].v)) {
				if (VLoc == l[left_CurPoint].v) {
					...
					left_HLoc = l[left_CurPoint].h;
					left_DeltaH = l[left_CurPoint + 1].h - l[left_CurPoint].h;
					left_HDir = (left_DeltaH > 0)?1:((left_DeltaH < 0)?-1:0);
					...
					left_CDir = (left_DeltaC > 0)?1:((left_DeltaC < 0)?-1:0);
				...
				if (VLoc == r[right_CurPoint].v) {
					...
					right_HLoc = r[right_CurPoint].h;
					right_DeltaH = r[right_CurPoint + 1].h - r[right_CurPoint].h;
					right_HDir = (right_DeltaH > 0)?1:((right_DeltaH < 0)?-1:0);
					right_DeltaH *= right_HDir;
					right_HRem = right_DeltaV/2;
					
					right_Color = r[right_CurPoint].c;
					right_DeltaC = r[right_CurPoint + 1].c - r[right_CurPoint].c;
					right_CRem = right_DeltaV/2;
					right_CDir = (right_DeltaC > 0)?1:((right_DeltaC < 0)?-1:0);
